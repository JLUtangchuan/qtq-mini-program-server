...
def add_user(username):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO user_table (nickname, isAdmin, isRoot, wait_for_admin) VALUES (?, ?, ?, ?)",
        (username, 0, 0, 0),
    )
    conn.commit()
    conn.close()
...
def auth_admin(username):
    ...
    cursor.execute(
        "UPDATE user_table SET wait_for_admin=0 WHERE nickname=?", (username,)
    )
    conn.commit()
    conn.close()
...
def wait_for_admin(username):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE user_table SET wait_for_admin=1 WHERE nickname=?", (username,)
    )
    conn.commit()
    conn.close()
...
def add_thing(title, year, author, content, image):
    ...
    cursor.execute(
        "INSERT INTO qtq_table (title, year, author, content, image, time) VALUES (?, ?, ?, ?, ?, datetime('now'))",
        (title, year, author, content, filename),
    )
    conn.commit()
    conn.close()
...
def query_things(year=None, sort_by_time=True):
    ...
    if year is None:
        ...
        cursor.execute(
            "SELECT * FROM qtq_table WHERE year=? ORDER BY time DESC", (year,)
        )
        res = cursor.fetchall()
    conn.close()
    return res
...
def load_thing(year, begin, num):
    ...
    return {
        'status' : 200,
        'data': data[begin:begin+num].tolist()
    }
